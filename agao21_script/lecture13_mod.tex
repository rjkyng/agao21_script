...
\begin{example}
  ...
  \begin{align*}
    \min_{\begin{subarray}{c} F\in\R \\ \BB\ff=F\bb_{s,t} \\ \matzero\leq\ff\leq\cc \end{subarray}} -F
    &= \min_{F;\ff\geq\matzero} \max_{\xx;\ss\geq\matzero} -F + \xx^\trp(F\bb_{s,t}-\BB\ff) + (\ff-\cc)^\trp\ss \\
    & \text{(Slater's condition $\implies$ strong duality)} \\
    -\max_{\begin{subarray}{c} F\in\R \\ \BB\ff=F\bb_{s,t} \\ \matzero\leq\ff\leq\cc \end{subarray}} F
    &= \max_{\xx;\ss\geq\matzero} \min_{F;\ff\geq\matzero} F(\bb_{s,t}^\trp\xx-1) + \ff^\trp(\ss-\BB^\trp\xx) -\cc^\trp\ss \\
    &= \max_{\begin{array}{c} \xx;\ss\geq\matzero \\ \bb_{s,t}^\trp\xx=1 \\ \ss\geq\BB^\trp\xx
    \end{array}} -\cc^\trp\ss \\
    \max_{\begin{subarray}{c} F\in\R \\ \BB\ff=F\bb_{s,t} \\ \matzero\leq\ff\leq\cc \end{subarray}} F
    &= \min_{\begin{array}{c} \xx;\ss\geq\matzero \\ \bb_{s,t}^\trp\xx=1 \\ \ss\geq\BB^\trp\xx
