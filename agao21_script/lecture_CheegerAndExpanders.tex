...
\begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.25]{fig/fig1_lecCheeger.png}
...
\end{itemize}
Today, we obtain a $\phi$-expander decomposition of quality $q = O(\phi^{-1/2})$. In a few lectures, we revisit the problem and obtain quality $q = O(\log n)$ which is almost tight. In practice, we mostly care about values $\phi \approx 1$.
...
\begin{itemize}
    ...
    \item Presents a \emph{cut} $S$ such that $\phi(S) \leq \sqrt{\phi}$.
\end{itemize}
...
\begin{align*}
       ...
       \iff \alpha &= \frac{\dd^\trp \vecone_S}{\dd^\trp\vecone} = \frac{\vol_G(S)}{\vol_G(V)}.
\end{align*}
...
\begin{itemize}
    ...
    \item Denominator: observe by straight-forward calculations that \begin{align*}
        \zz_S^\trp\DD\zz_S &= \vol_G(S) \cdot (1-\alpha)^2 + \vol_G(V \setminus S) \cdot (-\alpha)^2\\
        &= \vol_G(S) - 2\vol_G(S) \cdot \alpha + \vol_G(V) \cdot \alpha^2\\
        & = \vol_G(S) -   \frac{\vol_G(S)^2}{\vol_G(V)} \\
        & = \vol_G(S) - \vol_G(S) \cdot  \frac{\vol_G(S)}{\vol_G(V)}\\
        & \geq \frac{1}{2}\vol_G(S) = \frac{1}{2}\vecone_S^\trp\DD\vecone_S
    \end{align*}
    where we use the assumption that $\vol_G(S) \leq \vol_G(V)/2$.
\end{itemize}
\end{proof}
...
\begin{itemize}
    ...
    \item we \emph{center} $\zz$, that is we let $\zz_c = \zz - \alpha \vecone$ where $\alpha$ is chosen such that \begin{align*}
        ...
        \sum_{\zz_c(i) \geq 0} \dd(i) \geq \vol(V)/2
    \end{align*}
...
\begin{align}\label{eq:cheegerDefineDensityFun}
    \mathbb{P}[\tau = \ell] = \begin{cases}
        2 \cdot |\ell| &\ell \in [\zz_{sc}(1), \zz_{sc}(n)],
        \\
        0 &\text{otherwise}.
        \end{cases}
\end{align}
...
We have  $\mathbb{E}_{\tau}\left[\frac{\vecone_S^\trp \LL \vecone_S}{\vecone_S^\trp \DD \vecone_S}  \right] \leq \sqrt{2 \cdot \frac{\zz^\trp \LL \zz}{\zz^\trp \DD \zz}}.$
\end{claim}
\begin{proof}
...
\begin{align*}
    \mathbb{P}_{\tau}[e \in E(S_{\tau}, V \setminus S_{\tau})] &= \mathbb{P}_{\tau}[\zz_{sc}(i) < \tau \leq \zz_{sc}(j)] \\ &= \int_{t=i}^j 2|\tau|\; d\tau = \sgn{i} \cdot \zz_{sc}(i)^2 - \sgn{j} \cdot \zz_{sc}(j)^2.
\end{align*}
...
\begin{align*}
   \mathbb{E}_{\tau}[|E(S_{\tau}, V \setminus S_{\tau})|] &\leq \sum_{\{i,j\} \in E} |\zz_{sc}(i) - \zz_{sc}(j)| \cdot (|\zz_{sc}(i)| + |\zz_{sc}(j)|) \\
   &\leq \sqrt{\sum_{\{i,j\} \in E} (\zz_{sc}(i) - \zz_{sc}(j))^2 \cdot \sum_{\{i,j\} \in E} (\zz_{sc}(i) + \zz_{sc}(j))^2 }
\end{align*}
...
The first sum should look familiar by now: it is simply the Quadratic Laplacian Form $\sum_{\{i,j\} \in E} (\zz_{sc}(i) - \zz_{sc}(j))^2 = \zz_{sc}^\trp \LL \zz_{sc}$. \\

It is not hard to reason about the second term either, 
\[
\sum_{\{i,j\} \in E} (\zz_{sc}(i) + \zz_{sc}(j))^2 = \sum_{\{i,j\} \in E} \zz_{sc}(i)^2 + 2\zz_{sc}(i)\zz_{sc}(j) +\zz_{sc}(j)^2 \leq 2 \zz_{sc}^{\trp} \DD \zz_{sc}.
\]
...
\begin{align*}
    \mathbb{E}_{\tau}[\vecone_S^\trp \DD \vecone_{S}] &=\sum_{i \in V, \zz_{sc}(i) < 0} \dd(i) \cdot \mathbb{P}[i \in S \land S=S_{\tau}] + \sum_{i \in V, \zz_{sc}(i) \geq 0} \dd(i) \mathbb{P}[i \in S \land S \neq S_{\tau}]\\
...
\end{align*}
where we use that the event $\{i \in S \land S=S_{\tau}\}$ can be rewritten as the event $\{i < \tau \land \tau < 0\}$ (the other case is analogous). 
...
While \Cref{clm:cheegersInequInExpectation} only ensures our claim in expectation, this is already sufficient to conclude that there exists some set $S$ that satisfies the same guarantees deterministically. This is often called the \emph{probabilistic method of expectation} and can be seen straight-forwardly from the definition of expectation. We have thus proven the upper bound of Cheeger's inequality.
\end{proof}
