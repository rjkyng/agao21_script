...
In this chapter, we're going to frequently need to refer to vectors
...
It turns out that, if we have a solution $\ff_1$ to this problem for some
$\alpha_1 < 1$, then we can find a solution $\ff_2$ for some $\alpha_2 >
\alpha_1$. And, we can compute $\ff_1$ using a small number of Newton
steps, each of which will only require a Laplacian linear equation
solve, and hence is computable in $\Otil(m)$ time.
Concretely, for any $0 \leq \alpha_1 < 1$,
given the optimal flow at this $\alpha_1$, we will be able to compute
the optimal flow at $\alpha_2 = \alpha_1 + (1-\alpha_1)
\frac{1}{70\sqrt{m}}$.
This means that after $T = 70\sqrt{m}\log(1/\epsilon)$ updates, we have a solution
for $\alpha_{T} \geq 1-\epsilon$.
...
\begin{align}
  ...
  \grad V(\ff) = \BB^{\trp} \xx
  \\
  \tag*{"Lagrangian barrier gradient optimality"}
\end{align}

\subsection{Updates using Divergence}
\begin{align}
 \label{eq:divflow}
  \max_{\ddelta \in \R^E} & \quad 
    V(\ddelta+\ff)
  -
  (V(\ff)
  +
...
Thus, we can conclude, by the Karush-Kuhn-Tucker theorem from the
previous chapter, that $\ff+\ddelta$ is optimal for the
...
\begin{lemma}
  ...
  Suppose $\ff$ is the minimizer of Problem~\eqref{eq:barrierflow}
  (the Barrier Problem with parameter $\alpha$) and
  $\ddelta$ is the minimizer of Problem~\eqref{eq:divflow} (the
  Update Problem with parameters $\ff$ and $\alpha'$),
  then $\ff + \ddelta$ is optimal for Problem~\eqref{eq:barrierflow} 
  with parameter $\alpha+\alpha'$ (i.e. a new instance of the Barrier
  problem).
  
  I.e.  $\ff + \ddelta$ is the argmin for Problem~\eqref{eq:barrierflowupdated}.
\end{lemma}
...
\todo{point to proof, or give it}
\begin{lemma}
  \label{lem:argminsfromlocalagreement}
  Suppose $S \subseteq \R^n$ are a convex sets with $T \subseteq S$, and let $f, g: S \to R$ be convex
  functions.
  Let $\xx^* = \argmin_{\xx \in T} f(\xx)$.
  Suppose $f,g$ agree on a neighborhood of $\xx^*$ in $S$ (i.e. an
  open set containing $\xx^*$).
  Then $\xx^* = \argmin_{\xx \in T} f(\xx)$.
\end{lemma}
